    ...
    try{
      ...
      if(salsess.data.message=="succeed"){
        let perIndividualSalse=salsess.data.data
         const sortedData = perIndividualSalse.sort((a, b) => b.income- a.income);
         ...
         setRankedData(rankedDatas);
          ...
          setSalseloaded(true)
          setFilter(false)
          console.log("the Salsesssstore_++_", salse)
      }else{
      ...
      <div className=''>
        {dashboard ?
         <div className='flex flex-auto'>
             {dashboard && <Profile/>}
             <p className='font-semibold w-full text-center text-2xl pb-2 font-arial text-black border-b-2 rounded-lg'>Sales Performance</p>
             <DateRange/>
         </div>:
         <DateRange/>
        }
