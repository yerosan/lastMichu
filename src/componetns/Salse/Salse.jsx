...
const SalseDashboard = () => {
  ...
  const timeIntervalSalse=async(data)=>{
    ...
    try{
      ...
      // let weeklySalse= await axios.post(`${config.apiUrl}/salse/total`, data.weekly)
      ...
      if(totalSalse.data.message=="succeed" && salsePerUser.data.message=="succeed"){
        ...
        if(weeklySalse.data.message=='succeed' && monthlySalse.data.message=="succeed"){
          ...
          let AllData={totalSalse:dashboard, salsePerUser:salsePerUsers, monthlySalse:monthlySalses, weeklySalse:weeklySalses}
          console.log("this si dateVariation", dateVeriation)
          console.log("The Salse +++++----++++++++++", AllData)
          ...
          console.log("The Salse +++++----++ERAEEEE++++++++", weeklySalse.data.message)
        }
      }else{
        dispatch(intervalSalse({loading:false, error:totalSalse.data.message, data:null}))
        console.log("The Salse +++++----ERRRRRRpr++++++++++", totalSalse.data.message)
      }
    }catch(error){
