...
export default function DistrictPerUser() {
  ...
  const fetchCollectionPerUser=async()=>{
    ...
    try{
      const salesUsers=await axios.get(`${config.apiUrl}/salse/users`)
      const districtss= await axios.get(` ${config.apiUrl}/salse/districtList `)
      if(salesUsers.data.message=="succeed"){
         setRankedData(salesUsers.data.data);
         if(districtss.data.message=="succeed"){
          setDistrictList(districtss.data.data)
         }
          dispatch(collectionPerUser({loading:false,error:"", data:salesUsers.data.data}))
          setColl(salesUsers.data.data)
          setCollectionloaded(true)
      }else{
        dispatch(collectionPerUser({loading:false, error:salesUsers.data.message, data:null}))
      }
      ...
      <Table sx={{ minWidth: 450 }} stickyHeader aria-label="simple table">
        <TableHead>
          <TableRow style={styles} >
            ...
            <StyledTableCell align="center" style={styles} >User's District</StyledTableCell>
          </TableRow>
        </TableHead>
        <TableBody>
          {rankedData.map((row) => (
            ...
            <StyledTableCell component="th" scope="row">
              ...
              <StyledTableCell align="center">< UserRole userId={row.userId}/></StyledTableCell>
            </StyledTableRow>
          ))}
        </TableBody>
      </Table>
